...
export class GmailChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      logger.warn(
        { err: err.message || err },
        'Gmail: failed to connect — check that the Gmail API is enabled in your GCP project. Skipping.',
      );
      this.gmail = null;
      return;
    }
  ...
  async sendMessage(jid: string, text: string): Promise<void> {
    ...
    if (!meta) {
      ...
      try {
        ...
        logger.warn(
          { jid, err },
          'Failed to recover thread metadata from Gmail',
        );
      }
    }
